...
api_instance = dlxapi.ScoreWeightsApi(dlxapi.ApiClient(configuration))
...
> list[ScoreWeight] update_score_weights_for_portfolio(body)
...
try:
    # Updates Score Weights for portfolio
    api_response = api_instance.update_score_weights_for_portfolio(body)
    pprint(api_response)
except ApiException as e:
