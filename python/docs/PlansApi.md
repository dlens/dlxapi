...
# **export_whats_in**
> str export_whats_in(portfolio_id, portfolio_plan_ids, export_format, start_date=start_date, end_date=end_date)
...
try:
    # Export Whats In Comparison
    api_response = api_instance.export_whats_in(portfolio_id, portfolio_plan_ids, export_format, start_date=start_date, end_date=end_date)
    pprint(api_response)
...
> PortfolioPlanInsights get_insights_for_portfolio_plan(id, include_grades, start_date=start_date, end_date=end_date)
...
include_grades = false # bool | Whether to include grades in the response. (default to false)
start_date = 789 # int | Start time period for which the insights are computed for. (optional)
end_date = 789 # int | End time period for which the insights are computed for. (optional)

try:
    # Retrieves insights for portfolio plan.
    api_response = api_instance.get_insights_for_portfolio_plan(id, include_grades, start_date=start_date, end_date=end_date)
    pprint(api_response)
except ApiException as e:
 ...
 **include_grades** | **bool**| Whether to include grades in the response. | [default to false]
 **start_date** | **int**| Start time period for which the insights are computed for. | [optional] 
 **end_date** | **int**| End time period for which the insights are computed for. | [optional] 
...
> PortfolioPlanInsights get_value_insights_for_portfolio_plan(id, start_date=start_date, end_date=end_date)
...
try:
    # Retrieves value insights for portfolio plan.
    api_response = api_instance.get_value_insights_for_portfolio_plan(id, start_date=start_date, end_date=end_date)
    pprint(api_response)
except ApiException as e:
...
# **get_whats_in**
> WhatsIn get_whats_in(portfolio_id, portfolio_plan_ids, max_limit, start_date=start_date, end_date=end_date, offset=offset)
...
try:
    # Retrieves portfolioPlans with projects for WhatsIn
    api_response = api_instance.get_whats_in(portfolio_id, portfolio_plan_ids, max_limit, start_date=start_date, end_date=end_date, offset=offset)
    pprint(api_response)
except ApiException as e:
